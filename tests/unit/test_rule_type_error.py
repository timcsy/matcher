...
def test_ge_on_string_raises_rule_type_error():
    with pytest.raises(RuleTypeError) as ei:
        evaluate(Ge("role.seniority", 3), {"seniority": "資深"}, {})
...
def test_ge_on_bool_raises():
    ...
    with pytest.raises(RuleTypeError):
        evaluate(Ge("role.x", 1), {"x": True}, {})


def test_ge_on_int_still_works():
    assert evaluate(Ge("role.seniority", 3), {"seniority": 5}, {}) is True
    assert evaluate(Ge("role.seniority", 3), {"seniority": 2}, {}) is False

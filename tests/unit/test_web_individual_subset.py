...
def _sample_audit():
    return {
        ...
        "filter_trace": [
            {"role_id": "T01", "target_id": "C01", "qualified": True, "matched_rules": ["R001"]},
            {"role_id": "T01", "target_id": "C02", "qualified": False, "matched_rules": [], "failed_rule": "R001"},
            {"role_id": "T02", "target_id": "C01", "qualified": True, "matched_rules": ["R001"]},
            {"role_id": "T02", "target_id": "C02", "qualified": True, "matched_rules": ["R001"]},
        ],
        "allocation_trace": [
            {"step": 1, "role_id": "T01", "candidates": ["C01"], "random_index": 0,
             "chosen": "C01", "remaining_capacity_after": {"C01": 0, "C02": 1}},
            {"step": 2, "role_id": "T02", "candidates": ["C02"], "random_index": 0,
...
def test_filter_trace_subset_only_contains_role():
    sub = build_individual_audit_subset(_sample_audit(), "T01")
    assert len(sub["filter_trace_subset"]) == 2  # T01 has 2 entries in sample
    for entry in sub["filter_trace_subset"]:
        # 不應有 role_id 欄位（已經暗含在 subset 屬於該 role）
        # 但若保留 role_id 也合法——主要驗證條目來自 T01
        # 採用「target_id 集合 == 該 role 在 filter_trace 中的 targets」
...
def test_allocation_step_present_for_assigned():
    ...
    assert sub["allocation_step"]["role_id"] == "T01"


def test_role_id_not_found_raises():
    with pytest.raises(MatchRecordNotFound):
        build_individual_audit_subset(_sample_audit(), "T999")


def test_role_preferences_included():
    sub = build_individual_audit_subset(_sample_audit(), "T02")
    assert list(sub["role_preferences"]) == ["G1"]

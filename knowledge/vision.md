...
- spec-kit 與 knowie 初始化
- constitution（TDD、規格優先、繁體中文文件）
- 根公理與五條原則（見 `principles.md`）

尚未開始：技術棧選定、任何實作程式碼、實際媒合場景試行。
...
- [ ] 過濾與分配兩階段在程式碼結構上完全分離
- [ ] 介面已包含 `preferences?` 參數，但本階段僅實作 `preferences` 為空時的純抽籤分支
- [ ] 給定相同（規則 + 名單 + seed）能 100% 重現結果
- [ ] 「教師-班級配對」這個基準場景能用 CLI 跑通並輸出稽核紀錄
- [ ] 測試覆蓋核心：規則篩選、邊界（無人符合資格、人數不足）、分配可重現性（純抽籤分支）
